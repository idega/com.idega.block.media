...
public class MediaBusiness {
	...
	public static ICFile saveMediaToDB(ICFile file, int parentId, IWApplicationContext iwc) {
		try {
			...
			if (parentId == -1) { //add to root
				ICFile rootNode =
					(ICFile) iwc.getApplication().getIWCacheManager().getCachedEntity(
						com.idega.core.data.ICFileBMPBean.IC_ROOT_FOLDER_CACHE_KEY);
	...
	*/
	...
	public static FileTypeHandler getFileTypeHandler(IWContext iwc, String mimeType) throws MissingMimeTypeException {
		try {
			...
			ICFileTypeHandler typeHandler =
				(ICFileTypeHandler) cm.getFromCachedTable(
					ICFileTypeHandler.class,
					String.valueOf(getFileType(iwc, mimeType).getFileTypeHandlerID()));
	...
	public static ICFileType getFileType(IWContext iwc, String mimeType) throws MissingMimeTypeException {
		try {
			IWCacheManager cm = iwc.getApplication().getIWCacheManager();
			ICFileType type =
				(ICFileType) cm.getFromCachedTable(ICFileType.class, String.valueOf(getFileTypeId(iwc, mimeType)));
			return type;
		}
		catch (NullPointerException x) {
			throw new MissingMimeTypeException("The mimetype is missing", mimeType);
	...
	public static void saveMimeType(String mimeType, String description, int fileTypeId) {
		try {
			ICMimeType mime =
				((com.idega.core.data.ICMimeTypeHome) com.idega.data.IDOLookup.getHomeLegacy(ICMimeType.class))
					.createLegacy();
	...
	*/
	...
	public static boolean deleteMedia(int mediaId) {
		try {
			((com.idega.core.data.ICFileHome) com.idega.data.IDOLookup.getHomeLegacy(ICFile.class))
				.findByPrimaryKeyLegacy(mediaId)
				.delete();
			return true;
		}
	...
	public static ICFile saveMediaToDB(UploadFile uploadFile, int icFileParentId, IWContext iwc) {
		...
		try {
			...
			long time2 = System.currentTimeMillis();
			System.out.println(
				"MediaBusiness saveMediaToDB :" + (time2 - time1) + " ms for " + uploadFile.getSize() + " bytes");
			id = file.getID();
			uploadFile.setId(id);
			try {
